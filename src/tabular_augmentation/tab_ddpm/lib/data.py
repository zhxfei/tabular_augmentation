...
def prepare_fast_dataloader(
    ...
    dataloader = FastTensorDataLoader(D.x, D.y, batch_size=batch_size, shuffle=(split == 'train'))
    while True:
...
def read_pure_data(path, split='train', seed=1):
    from utils import get_auto_data, get_fraud_data, get_credit_card_data, get_fin_data, get_prp_auto_data
    if path == 'auto':
        x_train, y_train, x_test, y_test = get_auto_data(num_samples=50, seed=seed, positive_ratio=None)
